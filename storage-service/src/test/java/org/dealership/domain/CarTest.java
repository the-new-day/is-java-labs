...
class CarTest {
    ...
    void shouldExposePriceAndToggleAvailability() {
        ...
        CarModel model = DomainTestData.carModel(modelId, DomainTestData.brand(new BrandId(UUID.randomUUID())), selection, Set.of(ComponentType.WHEELS));
        Configuration configuration = DomainTestData.configuration(model, selection);

        Car car = DomainTestData.car(new CarId(UUID.randomUUID()), configuration, Color.BLUE, false);
