...
class ComponentVariantSelectionTest {
    ...
    void shouldAddVariantWithVariant() {
        CarModelId modelId = new CarModelId(UUID.randomUUID());
        ComponentVariant wheels = DomainTestData.variant(
                ComponentType.WHEELS, modelId, BigDecimal.valueOf(100));
        ComponentVariantSelection selection = DomainTestData.selection(wheels);

        ComponentVariant interior = DomainTestData.variant(
                ComponentType.INTERIOR, modelId, BigDecimal.valueOf(300));
